...
class CIFAR_100(BaseDataset):
    ...
    def load_annotations(self):
        ...
        for filename, gt_label in self.samples:
            ...
            data_infos.append(info)#鏋勫缓姣忎釜鍥剧墖瀵瑰簲鐨刧t label
        return data_infos
    ...
    def evaluate(
        ...
        if isinstance(metric, str):
            ...
            metrics = metric
        allowed_metrics = [
            'accuracy', 'precision', 'recall', 'f1_score', 'support', 'per_class_acc'
        ]
        eval_results = {}
        ...
        if 'per_class_acc' in metrics:
            ...
            per_class_acc2 = (confusion_matrix.diag() /
                                  confusion_matrix.sum(dim=1))
            for i in range(len(per_class_acc2)):
                    eval_results[self.CLASSES[i]] = per_class_acc2[i].item() * 100
